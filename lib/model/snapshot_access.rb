...
class SnapshotAccess
  ...
  def self.get_all_open_snapshots
    ...
    Snapshot.all.each do |snapshot|
      ...
      hash = {institution: institution[:name], account: account[:name], owner: account[:owner], investment: investment[:name], asset: investment[:asset], asset_class: investment[:asset_class], value: snapshot[:value], timestamp: snapshot[:timestamp], update_frequency: investment[:update_frequency], open_date: investment[:open_date], term: investment[:term]}
